...
const useCommandes = () => {
    ...
    const getCommandeById = async (id) => {
        try {
            setIsLoading(true);
            const response = await axios.get(`http://localhost:4000/commandes/${id}`);
            setCommandeSelected(response.data.payload);
        } catch (err) {
            setError(err.response?.data?.message || `Error fetching commande with id: ${id}`);
    ...
    const createCommande = async (commandeData) => {
        try {
            ...
            console.log("commandeData", commandeData)
             await axios.post("http://localhost:4000/commandes", commandeData);
            getAllCommandes()
        } catch (err) {
    ...
    const updateCommande = async (id, updatedData) => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.put(`http://localhost:4000/commandes/${id}`, updatedData);
            ...
            setError(err.response?.data?.message || `Error updating commande with id: ${id}`);
    ...
    const deleteCommande = async (id) => {
        try {
            ...
            setError(err.response?.data?.message || `Error deleting commande with id: ${id}`);
